...
const cleanInput = (n) => n.trim().replace(/\s+/g, ' ').replace(/ Mobile$/i, '').replace(/ Hold and Win$/i, '').replace(/ Megaways$/i, '');
...
async function reorderPopular() {
    ...
    if (aviator)
        console.log(`DEBUG: Found 'Aviator' in DB as: "${aviator.name}" (Provider: ${aviator.provider})`);
    else
        console.log(`DEBUG: 'Aviator' NOT FOUND in DB.`);
    let score = 20000;
    let foundCount = 0;
    const updates = [];
    ...
    for (const rawName of desiredOrder) {
        ...
        let match = allGames.find(g => {
            ...
            if (gNameClean.toLowerCase() === target.toLowerCase())
                return true;
            if (normalize(gNameClean) === targetNorm)
                return true;
            return false;
        });
        if (!match) {
            match = allGames.find(g => {
                ...
                if (tName.length > 4 && gName.includes(tName))
                    return true;
                if (gName.length > 4 && tName.includes(gName))
                    return true;
                return false;
            });
        }
    ...
    const spribeGames = allGames.filter(g => (g.provider.toLowerCase().includes('spribe') || g.name.toLowerCase().includes('spribe')) &&
        !matchedIds.has(g.id));
...
//# sourceMappingURL=order-popular.js.map