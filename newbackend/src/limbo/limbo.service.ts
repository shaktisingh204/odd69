...
type LimboWalletField = 'balance' | 'cryptoBalance' | 'casinoBonus';
type LimboAllocation = {
  walletField: LimboWalletField;
  walletLabel: string;
  amount: number;
};
...
export class LimboService {
  ...
  private getWalletFieldLabel(walletField: LimboWalletField) {
    if (walletField === 'casinoBonus') return 'Casino Bonus Wallet';
    return walletField === 'cryptoBalance' ? 'Crypto Wallet' : 'Main Wallet';
  }

  private getPrimaryWalletField(walletType: string): LimboWalletField {
    return walletType === 'crypto' ? 'cryptoBalance' : 'balance';
  }

  private mapWalletFieldToPaymentMethod(walletField: LimboWalletField) {
  ...
  private buildAllocations(
    ...
    if (walletType === 'crypto') {
      const allocations: LimboAllocation[] = [{
    ...
    for (const bet of losingBets) {
      ...
      await this.prisma.transaction.create({
        data: {
          ...
          remarks: `Limbo loss on round #${roundId}`,
          createdAt: new Date(),
          updatedAt: new Date(),
        },
      });
    ...
    try {
      ...
      await this.prisma.transaction.create({
        data: {
          ...
          remarks: `Limbo cashout on round #${bet.roundId} at ${multiplier.toFixed(2)}x`,
          createdAt: new Date(),
          updatedAt: new Date(),
        },
      });
  ...
  async placeBet(
    ...
    await this.prisma.transaction.create({
      data: {
        ...
        remarks: `Limbo bet placed on round #${roundId}`,
        createdAt: new Date(),
        updatedAt: new Date(),
      },
    });
