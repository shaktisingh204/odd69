...
export default function GameCard({ game }: { game: any }) {
  ...
  const launch = () => {
    const prov = game.provider || game.providerCode || game.providerSlug || "";
    window.location.href = `/casino/play/${game.id}?provider=${encodeURIComponent(prov)}&name=${encodeURIComponent(name)}`;
  };

  return (
    <button onClick={launch} aria-label={`Play ${name}`} className="group relative block w-full cursor-pointer text-left outline-none">
      <div className="relative aspect-[3/4] overflow-hidden rounded-2xl ring-1 ring-white/[0.06] transition-transform duration-300 group-hover:-translate-y-1.5"
        ...
        <div className="absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100 group-focus-visible:opacity-100" style={{ boxShadow: "inset 0 0 0 2px rgba(255,122,26,0.85)" }} />
